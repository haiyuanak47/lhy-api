$(function() {
	...
	$('#globalQueryParams_add').click(function () {
		...
		$('#globalQueryParams_parent .select2_tag_new').last().change(globalQueryParamsChange);
	...
	/**
	 * 全局请求参数 select change
	 */
	var globalQueryParamsChange =function () {
		var globalQueryParams_item = $(this).parent().parent();
		var value = globalQueryParams_item.find('.value');
		var desc = globalQueryParams_item.find('.desc');
		var type = globalQueryParams_item.find('.type');
		var notNull = globalQueryParams_item.find('.notNull');
		var key = $(this).find("option:selected").attr("data-key");
		//当前项 全局请求参数json
		var currQueryParamsObj = new Object();
		for(var p in globalParamsJson){
			var mGlobalParamsJson = globalParamsJson[p];
			if(mGlobalParamsJson.name==key){
				currQueryParamsObj = mGlobalParamsJson;
			}
		}
		value.val($(this).val());
		desc.val(currQueryParamsObj.desc);
		type.val(currQueryParamsObj.type);
		notNull.val(currQueryParamsObj.notNull);
	}
	//读取的全局参数 change事件
	$('#globalQueryParams_parent .key').change(globalQueryParamsChange);
	/**
	 * 全局请求参数，删除一行
	 */
	...
	$('#responseDatatypeId').select2({
		ajax: {
			...
			processResults: function (data, params) {
				...
				var arr = data.data;
				for(i in arr){
					itemList.push({id: arr[i].id, text: arr[i].name+' '+ arr[i].about})
				}
				return {
