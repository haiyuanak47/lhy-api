	...
	<div class="content-wrapper">
        ...
        <section class="content">
            <form class="form-horizontal" id="ducomentForm" >
                ...
                <div class="box box-primary">
                    <div class="box-header with-border">
                        <h3 class="box-title">全局请求参数</h3>
                        <div class="box-tools pull-right">
                            <button type="button" class="btn btn-box-tool" id="globalQueryParams_add" ><i class="fa fa-plus"></i></button>
                        </div>
                    </div>

                    <div id="globalQueryParams_example" style="display: none;" >
                        <div class="form-group globalQueryParams_item" >
                            <label class="col-sm-1 control-label">参数</label>
                            <div class="col-sm-2 item">
                                <select class="form-control select2_tag_new key" >
                                    <option value=""></option>
                                    <#if projectGlobalQueryParams?exists>
                                        <#list projectGlobalQueryParams as item>
                                            <option value="${item.value}" data-key="${item.name}">${item.name}</option>
                                        </#list>
                                    </#if>
                                </select>
                            </div>
                            <#--<label class="col-sm-1 control-label">Value</label>-->
                            <div class="col-sm-5 item" style="display: none">
                            ...
                            <label class="col-sm-1 control-label">类型</label>
                            <div class="col-sm-2 item">
                                <select class="form-control type" style="width: 100%;" disabled="disabled">
                                    <#list QueryParamTypeEnum as item>
                                        <option value="${item}" >${item}</option>
                                    </#list>
                                </select>
                            ...
                            <div class="col-sm-2 item">
                                <select class="form-control notNull" style="width: 100%;" disabled="disabled">
                                    <option value="true" >必填</option>
                                    <option value="false" >非必填</option>
                                </select>
                            </div>
                    ...
                    <div class="box-body" id="globalQueryParams_parent" >
                        <#if globalQueryParams?exists>
                            <#list globalQueryParams as item>
                                <#assign key = item['key'] />
                                ...
                                <#assign notNull = '' />
                                <#if projectGlobalQueryParams?exists>
                                    <#list projectGlobalQueryParams as  projectQueryParams>
                                        <#if projectQueryParams.name == key>
                                            <#assign type = projectQueryParams['type'] />
                                            <#assign desc = projectQueryParams['desc'] />
                                            <#assign notNull = projectQueryParams['notNull'] />
                                        </#if>
                                    </#list>
                                <#else>
                                ...
                                <div class="form-group globalQueryParams_item" >
                                    ...
                                    <div class="col-sm-2 item">
                                        <select class="form-control select2_tag key" >
                                            ...
                                            <#if projectGlobalQueryParams?exists>
                                                <#list projectGlobalQueryParams as item>
                                                    <option value="${item.value}" data-key="${item.name}" <#if item.name == key>selected</#if> >${item.name}</option>
                                                </#list>
                                            <#else>
                                                <option value="${value}" data-key="${key}" selected>${key}</option>
                                            </#if>
                                        </select>
                                    </div>
                                   <#-- <label class="col-sm-1 control-label">Value</label>-->
                                    ...
                                    <label class="col-sm-1 control-label">说明</label>
                                    <div class="col-sm-2 item">
                                        <input type="text" class="form-control desc" value="${desc}"  readonly>
                                    </div>
                                    <label class="col-sm-1 control-label">类型</label>
                                    <div class="col-sm-2 item">
                                        <select class="form-control select2_tag type" style="width: 100%;" disabled="disabled">
                                            <#list QueryParamTypeEnum as item>
                                                <option value="${item}" <#if type == item>selected</#if> >${item}</option>
                                            </#list>
                                        </select>
                                    </div>
                                    <div class="col-sm-2 item">
                                        <select class="form-control select2_tag notNull" style="width: 100%;" disabled="disabled">
                                            <option value="true" <#if notNull == "true" >selected</#if> >必填</option>
                                            <option value="false" <#if notNull == "false" >selected</#if> >非必填</option>
                                        </select>
                                    </div>
                                    <button type="button" class="col-sm-1 btn btn-box-tool delete" ><i class="fa fa-fw fa-close"></i></button>
                                </div>
                    ...
                    <div class="box-body" >
                        ...
                        <div class="col-sm-4 item">
                            ...
                            <#if responseDatatype?exists>
                                <option value="${responseDatatype.id}">${responseDatatype.name} ${responseDatatype.about}</option>
                            </#if>
                            </select>
                        </div>（温馨提示:输入类型关键字）
